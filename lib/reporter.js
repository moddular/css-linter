...
module.exports = function(args) {
	var cp = require('child_process'),
		...
		getUser = function(line) {
			// parse a line of git blame output to extract the user name
	...
	return {
		...
		loadFile: (args.blame) ? function(path) {
			var self = this;
			// git blame may be too big for the default buffer size
			cp.exec('git blame ' + path, {maxBuffer: 1024 * 1024}, function(err, stdout, stderr) {
				// keep an array of the lines in git blame for each file
		...
		accuse: (args.blame) ? function() {
			...
			// take a list, apply a transform to it, then reduce to find the max value
			var getMax = function(items, transform) {
				return items.map(transform).reduce(function(prev, curr) { return Math.max(prev, curr); });
			};
