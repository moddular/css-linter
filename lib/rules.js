...
module.exports = [
	...
	},
	{
		...
		name: 'If using vendor prefixes, -webkit MUST be first',
			...
			parser.addListener('property', function(e) {
				...
				if (properties[name]) {
					if (found[properties[name]] && name.indexOf('-webkit') === 0) {
						reporter.report('The property ' + name + ' MUST be the first vendor prefix used', e.property.line, e.property.col, rule);
					} else {
		...
		init: function(parser, reporter) {
			...
			parser.addListener('endrule', function(e) {
				if (gradients.length && gradients[0] !== 'webkit') {
					reporter.report('-webkit prefixed gradient MUST appear before other vendor prefixes', e.selectors[0].line, e.selectors[0].col, rule);
				}
			});
		}
	},
	...
	},
	{
		...
		desc: 'All properties must appear on their own line with a single hard tab indentation',
		browsers: 'All',
		init: function(parser, reporter) {
			var rule = this;
			...
			parser.addListener('property', function(e) {
				if (e.col !== 2) {
					reporter.report(e.property.text + ' properties MUST be indented by 1 hard tab', e.property.line, e.property.col, rule);
				}
			});
		}
	}
