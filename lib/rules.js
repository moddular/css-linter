...
module.exports = [
			...
			parser.addListener('property', function(e) {
				...
				while (i < n) {
					if (parts[i].type === 'color') {
						rgb = null;
						if (parts[i].text.match(/^[a-z]+$/)) {
							reporter.report('The value ' + parts[i].text + ' MUST be written in hex format', parts[i].line, parts[i].col, rule);
						} else if (parts[i].text.toLowerCase() !== parts[i].text) {
		...
		init: function(parser, reporter) {
			...
			parser.addListener('endrule', function(e) {
				Object.keys(found).forEach(function(prop) {
					...
					if (!ok) {
						reporter.report(prop + ' definitions MUST be in order ' + prefixes.map(function(val) {
							return (val === '') ? 'non-prefixed' : '-' + val;
						}).join(', '), e.selectors[0].line, e.selectors[0].col, rule);
					}
				});
			});
		}
	},
	{
		...
		init: function(parser, reporter) {
			...
			parser.addListener('property', function(e) {
				...
				} else if (/-(moz|o|webkit)(?:-(?:linear|radial))-gradient/i.test(e.value)) {
			...
			parser.addListener('endrule', function(e) {
				...
				if (n) {
					...
					if (!ok) {
						reporter.report('Gradient definitions MUST be in order -webkit (old style, then new), -moz, -o and non-prefixed. -ms-linear-gradient is NOT required', e.selectors[0].line, e.selectors[0].col, rule);
					}
				}
			});
		}
	...
	{
		id: 'use-paths-relative-from-root',
	...
	},
	{
		...
		init: function(parser, reporter) {
			...
			parser.addListener('startrule', function(e) {
				...
				if (depth > 2) {
					msg = (depth - 2) === 1 ? '1 hard tab' : (depth - 2) + ' hard tabs';
					n = e.selectors.length;
					while (n--) {
						if (e.selectors[n].col !== depth - 1) {
							reporter.report(e.selectors[n].text + ' selector MUST be indented by ' + msg, e.selectors[n].line, e.selectors[n].col, rule);
						}
					}
				}
			});
			...
			parser.addListener('property', function(e) {
				var msg = (depth === 2) ? '1 hard tab' : (depth - 1) + ' hard tabs';
				
				if (e.col !== depth) {
					reporter.report(e.property.text + ' property MUST be indented by ' + msg, e.property.line, e.property.col, rule);
				}
	...
	},
	{
		id: 'borders',
		name: 'Remove borders',
		desc: 'Border width of 0 MUST be used to remove borders from an element',
		browsers: 'All',
		init: function(parser, reporter) {
			var rule = this;
			...
			parser.addListener('property', function(e) {
				...
				if (name.match(/^border(-(left|right|top|bottom))?$/) && e.value.text.toLowerCase().indexOf('none') !== -1) {
					reporter.report('The property ' + e.property.text + ': ' + e.value.text + '; MUST be written as ' + e.property.text + ': 0;', e.property.line, e.property.col, rule);
				}
			});
		}
	},
	{
		...
		desc: 'Background position should not use keywords',
		browsers: 'All',
		init: function(parser, reporter) {
			var rule = this,
				positions = {left: 1, right: 1, center: 1};
			...
			parser.addListener('property', function(e) {
				...
				if (e.property.text === 'background' || e.property.text === 'background-position') {
					while (n--) {
						if (parts[n].type === 'identifier' && parts[n].text in positions) {
							reporter.report('background-attachment should not use position keywords: ' + e.value.text, e.property.line, e.property.col, rule);
						}
					}
				}
			});
